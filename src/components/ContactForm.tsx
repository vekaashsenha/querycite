  ...
  if (submitted) {
    return (
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 text-emerald-900">
        ...
        <p className="mt-3 text-sm leading-6">Your message has been received for review. The QueryCite team will use this context to understand your AI visibility goals.</p>
    ...
    <form onSubmit={handleSubmit} className="grid gap-4 rounded-3xl border border-white/70 bg-white/85 p-6 shadow-lg">
      {[
        ...
        <label key={String(label)} className="grid gap-2 text-sm font-semibold text-slate-700">
          {label}
          <input required={Boolean(required)} type={String(type)} className="min-h-12 rounded-2xl border border-slate-200 bg-white px-4 outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100" />
        </label>
      ))}
